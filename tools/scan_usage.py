...
def scan_file(file_path: Path) -> DeepUsageScanner | None:
...
def get_installed_version(project_path: Path) -> str | None:
    ...
    try:
        ...
        result = subprocess.run(
            ["pip", "show", "deep-timeseries"], capture_output=True, text=True, cwd=project_path
        )
...
def print_text_report(results: dict):
    ...
    print("\n" + "=" * 70)
    print("Deep-TimeSeries Usage Report")
    print("=" * 70)
...
def main():
    parser = argparse.ArgumentParser(description="Scan projects for deep-timeseries usage patterns")
    parser.add_argument(
        "project_path",
        type=Path,
        nargs="?",
    ...
    parser.add_argument("--version", action="store_true", help="Also check installed version")
