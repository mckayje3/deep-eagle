...
def show():
    ...
    with col4:
        improvement = (
            (history["val_loss"][0] - history["val_loss"][-1]) / history["val_loss"][0] * 100
        )
    ...
    with col1:
        ...
        fig_hist.add_trace(
            go.Histogram(
                x=history["train_loss"], name="Train Loss", opacity=0.7, marker_color="blue"
            )
        )
        fig_hist.add_trace(
            go.Histogram(x=history["val_loss"], name="Val Loss", opacity=0.7, marker_color="orange")
        )
    ...
    with col2:
        ...
        st.dataframe(
            train_stats.style.format({"Train Loss": "{:.4f}", "Val Loss": "{:.4f}"}),
    ...
    with col2:
        # Export predictions
        predictions_df = pd.DataFrame({"actual": actual, "predicted": predicted, "error": errors})
    ...
    with col3:
        ...
        st.download_button(
            label="📥 Download Metrics", data=metrics_csv, file_name="metrics.csv", mime="text/csv"
        )
